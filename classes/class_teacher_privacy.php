...
class teacher_privacy
{
	...
	{
		$select=mysql_query("SELECT Maintain_emailid,Maintain_phonenumber FROM teachers_secondary_info WHERE Emailid='$id' ");
	...
	public function validate_password($pass1)
	{
		...
		{
			$select=mysql_query("SELECT Password FROM teachers_primary_info WHERE Emailid='$_SESSION[emailid]' ");
			$row=mysql_fetch_array($select);
	...
	{
		$update="UPDATE teachers_secondary_info SET Maintain_emailid='$this->_emailid_privacy',Maintain_phonenumber='$this->_phno_privacy' WHERE Emailid='$id' ";
		if(!mysql_query($update))
		{
			 error_log("\nUnable to update privacy in teacher_privacy".mysql_error(),3, "../logged errors/e.log");
		}
	}
}
?>